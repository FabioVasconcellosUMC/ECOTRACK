      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-bg-border">
            <th class="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-text-secondary">ID</th>
            ...
            <th class="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-text-secondary">Quantidade</th>
        ...
        <tbody>
          <tr v-if="carregando">
            <td colspan="7" class="text-center py-8 text-text-secondary">Carregando...</td>
          </tr>
          <tr v-else-if="lotesFiltrados.length === 0">
            <td colspan="7" class="text-center py-8 text-text-secondary">Nenhum lote encontrado</td>
          </tr>
          <tr v-for="lote in lotesFiltrados" :key="lote.id"
            class="border-b border-bg-border hover:bg-bg-border transition-colors">
            <td class="px-4 py-3 text-sm font-mono text-accent">#{{ lote.id }}</td>
            <td class="px-4 py-3">
              <span class="px-2 py-1 rounded-full text-xs font-semibold border" :class="tipoBadge(lote.tipoResiduo)">
                {{ lote.tipoResiduo }}
              </span>
            </td>
            <td class="px-4 py-3 text-sm text-text-primary">{{ lote.empresaGeradora?.razaoSocial }}</td>
            <td class="px-4 py-3 text-sm font-mono text-text-secondary">{{ lote.quantidade }} {{ lote.unidade }}</td>
            <td class="px-4 py-3">
...
const tipoBadge = (tipo) => {
  const map = {
    'Eletrônico': 'text-purple-400 border-purple-400/30 bg-purple-400/10',
    'Orgânico': 'text-yellow-400 border-yellow-400/30 bg-yellow-400/10',
    'Químico': 'text-orange-400 border-orange-400/30 bg-orange-400/10',
    'Perigoso': 'text-danger border-danger/30 bg-danger/10',
    'Hospitalar': 'text-pink-400 border-pink-400/30 bg-pink-400/10',
    'Reciclável': 'text-accent border-accent/30 bg-accent/10',
  }
...
const fecharModal = () => {
  ...
  form.value = { tipoResiduo: '', quantidade: '', empresaId: '', descricao: '' }
  erro.value = ''
}
